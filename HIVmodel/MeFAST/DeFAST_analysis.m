...
%       y_var = output variable of interest in the order defined in the ODE
%       model   
 load(filename);
...
for i =1:K % loop over parameter
    for n = 3:NR-1 % loop over NR
             %when NR is too small, not much statistics to work with
    [h(n-2,i),p(n-2,i),dist(n-2,i)]=...
        eval(['kstest2(range',type,'(1:n,',num2str(time_id),',i,',...
...
for j = 5:NR % Start with NR = 5, which is used in Marino et al. 
    ...
    [c,m,h,nms] = multcompare(stats); %Tukey procedure 
    ...
    %=======Student t-test, Wilcoxon, Permutation test ========%
    ...
    p_permute(i,j-4) = permutationTest(sens_par, dummy,1000,...
        'plotresult', 0);
     end % end i parameter loop
   
   % Count number of significant parameters  
   n_welch(j-4) = sum(welch(:,j-4));
   n_wilcox(j-4) = sum(wilcox(:,j-4)); 
   n_permute(j-4) = sum(p_permute(:,j-4)<alpha/K); 
   
   % compute average sensitivity indice
   % sort in descending order
   sort_efast= ...
       eval(['squeeze(mean(range',type,'(1:j,1,:,',num2str(y_var),')))']);
...
end %end NR loop here



%% ------ Bar plot of Average sensitivity index -------- %
mean_S= eval(['squeeze(mean(range',type,'(:,1,:,',num2str(y_var),')))']);
